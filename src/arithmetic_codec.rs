...
impl FloatCodec {
    ...
    pub fn encode(&self, input: &str) -> BitHandler {
        ...
        while cur_index < input.len() {
            ...
            assert!(l < u);
            while l > 0.5 || u < 0.5 || l < 0.5 && l > 0.25 && u > 0.5 && u < 0.75 {
                if l > 0.5 {
                ...
                } else if l > 0.25 && l < 0.5 && u > 0.5 && u < 0.75 {
                    (l, u) = (2.0 * (l - 0.25), 2.0 * (u - 0.25));
                    e3_cnt += 1;
                }
            }
            println!("c: {c} fx_l:{fx_l:.4} fx_r:{fx_r:.4}, l:{l:.4}, u:{u:.4} e3_cnt:{e3_cnt}");
            println!("{handler:?}");
            cur_index += 1;
        }
        handler.write_bit_back(true);
    ...
    fn decode(&self, handler: BitHandler) -> String {
        ...
        while !reader.is_empty() {
            ...
            println!(
                "cur_label:{cur_label}({}), l:{l:.4}, u:{u:.4} proportion: {proportion:.4}",
                reader.cur_bit()
            );
            ...
            while l > 0.5 || u < 0.5 || l < 0.5 && l > 0.25 && u > 0.5 && u < 0.75 {
                ...
                if l > 0.5 {
                    (l, u) = (2.0 * (l - 0.5), 2.0 * (u - 0.5));
                } else if u < 0.5 {
                    (l, u) = (2.0 * l, 2.0 * u);
                } else if l < 0.5 && l > 0.25 && u > 0.5 && u < 0.75 {
                    (l, u) = (2.0 * (l - 0.25), 2.0 * (u - 0.25));
                    if cur_label > 0.5 {
                        cur_label -= 0.5;
                    } else {
...
mod tests {
    ...
    fn test_encode() {
        ...
        let mut codec = FloatCodec::new_with_freq(8, freq);
        let input = "acbaa";
